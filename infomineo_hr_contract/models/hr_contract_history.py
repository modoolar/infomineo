...
class ContractHistory(models.Model):
    ...
    def init(self):
        """Override of the original method to include 'freelance' employee type."""
        tools.drop_view_if_exists(self.env.cr, self._table)
        # Reference contract is the one with the latest start_date.
        table = self._table
        columns = self._get_fields()
        sql_query = f"""CREATE or REPLACE VIEW {table} AS (
            ...
            -- Overridden part
            WHERE   employee.employee_type IN ('employee', 'student', 'trainee', 'freelance')
        )"""
